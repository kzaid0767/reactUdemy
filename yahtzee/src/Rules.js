...
class FullHouse extends Rule {
  evalRoll = dice => {
    // dice has 3 of kind and 2 of another
    const d = new Set(this.freq(dice))
    return (d.size === 2) && (d.has(3) && d.has(2)) ? this.score : 0;
  };
}
...
class SmallStraight extends Rule {
  ...
  evalRoll = dice => {
    ...
    //small straight must be 4 different dice & no (5 and 6) or no (1and6) or no(1and2)
    return d.size === 4 && ((!d.has(5) || !d.has(6)) || (!d.has(1) || !d.has(6)) || (!d.has(1) || !d.has(2))) ? this.score : 0
  }
}
